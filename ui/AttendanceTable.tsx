...
const StatusIcon = ({ time, date, runtime }: { time: string; date: string; runtime?: any }) => {
  ...
  if (minutesRemaining > 0 && minutesRemaining <= startingSoonThreshold) {
    return <span className="flex w-3 h-3 bg-blue-500 rounded-full mx-auto"></span>;
  } else if (minutesRemaining > startingSoonThreshold && minutesRemaining <= 0) {
    return <span className="flex w-3 h-3 bg-green-500 rounded-full mx-auto"></span>;
  } else if (minutesRemaining > 0 && minutesRemaining <= endingSoonThreshold) {
    return <span className="flex w-3 h-3 bg-yellow-500 rounded-full mx-auto"></span>;
  } else if (minutesRemaining <= 0 && now <= endTime) {
    return <span className="flex w-3 h-3 bg-red-500 rounded-full mx-auto"></span>;
  } else {
    return null; // Default: No status icon if not within the defined thresholds
  }
};
